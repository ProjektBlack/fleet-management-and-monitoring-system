...
router.delete("/trucks/:id", async (req, res) => {
  try {
    ...
    res.status(204).json({
      message:
        "Truck and its associated trips and expenses deleted successfully.",
    });
  } catch (error) {
    console.error(error);
...
router.delete("/trips/:id", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Trip deleted and removed from references." });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Server error." });
  }
  ...
  try {
    ...
    if (!trips || trips.length === 0) {
      return res.status(404).json({
        message: `There are no completed trips for ${formattedMonth} ${year}.`,
      });
    }
...
router.get("/trips/status/completed/:year", async (req, res) => {
  try {
    ...
    const trips = await Trip.find({ status: "Done", year: year });
    if (!trips || trips.length === 0) {
      return res
        .status(404)
        .json({ message: `There are no completed trips for ${year}.` });
    }
    res.status(200).json(trips);
  } catch (error) {
    console.error(error.message);
...
router.get("/trips/:truck/:year/:month", async (req, res) => {
  try {
    ...
    if (!trips) {
      res
        .status(404)
        .json({ message: "There are no trips for that year and month." });
    }
    res.status(200).json(trips);
  } catch (error) {
    console.error(error.message);
...
router.post("/expenses/monthly", (req, res) =>
  createRecord(MonthlyExpense, req, res)
);
...
router.put("/expenses/monthly/:id", (req, res) =>
  updateRecord(MonthlyExpense, req, res)
);
...
router.delete("/expenses/monthly/:id", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Expense deleted and removed from references." });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Server error." });
  }
...
router.post("/expenses/yearly", (req, res) =>
  createRecord(YearlyExpense, req, res)
);
router.get("/expenses/yearly", (req, res) => getAllRecords(YearlyExpense, res));
router.get("/expenses/yearly/:id", (req, res) =>
  getSingleRecord(YearlyExpense, req, res)
);
router.put("/expenses/yearly/:id", (req, res) =>
  updateRecord(YearlyExpense, req, res)
);
...
const updateRecord = async (model, req, res) => {
  try {
    ...
    res
      .status(200)
      .send({ message: `${model.modelName} updated successfully.` });
  } catch (error) {
    console.error(error.message);
    res.status(500).send({ message: error.message });
  }
